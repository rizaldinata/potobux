...
<body class="bg-gray-100 text-gray-800 font-sans antialiased min-h-screen flex flex-col">
    ...
    <nav class="bg-white shadow">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            <h1 class="text-2xl font-bold text-blue-600">胴 Potobux Studio</h1>
            <div class="flex items-center gap-4">
                @auth
                    ...
                    <button onclick="openUploadModal()"
                        class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded font-semibold transition shadow-sm">Upload
                        Bingkai +</button>
                @else
                    <a href="{{ route('login') }}" class="text-sm font-medium text-gray-700 hover:underline">Login</a>
                    <a href="{{ route('login') }}"
                        class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded font-semibold transition shadow-sm">Upload
                        Bingkai +</a>
                @endauth
            </div>
        </div>
    ...
    <main class="flex-grow max-w-7xl mx-auto px-4 py-8 w-full flex flex-col md:flex-row gap-8">
        ...
        <div class="w-full md:w-2/3 flex flex-col items-center">
            <h2 id="capture-status"
                class="text-xl font-bold text-blue-600 mb-2 py-1 px-4 bg-blue-100 rounded-full shadow-inner">Pilih
                Bingkai & Bersiaplah!</h2>

            <div class="camera-container shadow-2xl ring-4 ring-white bg-gray-100 relative">
                <video id="webcam" autoplay playsinline class="hidden"></video>
                <img id="overlay-frame" src="" alt="" class="hidden">
                <canvas id="live-studio-canvas" class="w-full h-full object-contain absolute inset-0"></canvas>
            </div>

            <div class="mt-6 flex flex-col items-center w-full max-w-sm">
                <div class="flex items-center gap-4 mb-4">
                    <button id="prev-slot-btn" onclick="goToPreviousSlot()"
                        class="hidden bg-gray-200 hover:bg-gray-300 text-gray-700 p-4 rounded-full transition shadow-md">
                    ...
                    <button id="capture-btn"
                        class="bg-blue-600 hover:bg-blue-700 text-white rounded-full w-20 h-20 flex items-center justify-center shadow-lg transition transform hover:scale-105 hover:animate-pulse focus:outline-none">
                ...
                <div id="review-controls" class="hidden w-full flex gap-4 mt-2">
                    <button onclick="retakeCurrentSlot()"
                        class="flex-1 py-3 text-red-600 font-bold border-2 border-red-600 bg-white rounded-xl hover:bg-red-50 transition shadow-sm">Ulangi
                        Pose Ini</button>
                    <button onclick="acceptCurrentSlot()"
                        class="flex-1 py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg transition flex justify-center items-center gap-2">
                        Bagus, Lanjut! <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div id="result-container"
                class="mt-8 hidden flex flex-col items-center bg-white p-8 rounded-xl shadow-2xl w-full max-w-md border-t-8 border-green-500">
                <h3 class="text-2xl font-black mb-4 text-gray-800 text-center">Gambar Final Potobux! 脂</h3>
                <img id="result-image" class="w-full h-auto rounded-lg shadow-md object-contain mb-6 border">
                <div class="flex flex-col sm:flex-row gap-4 w-full">
                    <a id="download-btn"
                        class="flex-1 text-center bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg cursor-pointer font-bold transition shadow-lg text-lg">踏
                        Download Foto</a>
                    <button onclick="retake()"
                        class="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-lg font-bold transition shadow">萄
                        Ulangi Sesi</button>
                </div>
            </div>
        </div>
    ...
    <div id="upload-modal" class="fixed inset-0 bg-black/80 hidden z-50 overflow-y-auto backdrop-blur-sm">
        <div class="flex items-center justify-center min-h-screen p-4">
            <div class="bg-white rounded-2xl w-full max-w-4xl p-8 shadow-2xl flex flex-col md:flex-row gap-8">
                <div class="w-full md:w-1/3 flex flex-col gap-4">
                    ...
                    <form id="upload-form" class="flex flex-col gap-4 flex-grow">
                        ...
                        <div>
                            ...
                            <input type="file" id="frame-file" accept="image/png" onchange="previewUpload(this)"
                                required class="w-full text-sm p-1 border rounded-lg bg-gray-50 cursor-pointer">
                            <p class="text-[10px] text-gray-500 mt-1">Pastikan berpori-pori/transparan di lubang tempat
                                foto.</p>
                        </div>
                        ...
                        <div class="border-2 border-indigo-100 rounded-lg p-4 bg-indigo-50/30 mt-2 flex-grow">
                            ...
                            <p class="text-xs text-indigo-700/80 mb-4">Gunakan layar visual di samping untuk menarik,
                                mengecilkan, dan memutar (rotasi) posisi kotak tangkapan kamera.</p>
                            ...
                            <button type="button" onclick="removeActiveSlot()"
                                class="w-full text-red-500 hover:bg-red-50 font-semibold py-2 rounded border border-red-200 transition text-sm mt-1">Hapus
                                Kotak Terpilih</button>
                        </div>
                ...
                <div
                    class="w-full md:w-2/3 flex flex-col items-center justify-center bg-gray-900 rounded-xl p-4 overflow-hidden shadow-inner relative">
                    <p class="text-sm font-bold text-indigo-300 mb-4 tracking-widest"><span
                            class="animate-pulse mr-1">閥</span> VISUAL CANVAS EDITOR</p>
                    ...
                    <div class="mt-6 flex justify-center text-xs text-gray-400 max-w-sm text-center">
                        Jika Anda mengeklik "Tambah Kotak Scanner", geser balok biru yang muncul di layar, lalu putar
                        tangkainya menyesuaikan jajaran giring kemiringan *Polaroid* Anda.
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        const liveStudioCanvas = document.getElementById('live-studio-canvas');
        const liveCtx = liveStudioCanvas ? liveStudioCanvas.getContext('2d') : null;
        ...
        async function loadFrames() {
            try {
                ...
                if (json.data && json.data.length > 0) {
                    ...
                    framesList.innerHTML =
                        '<p class="text-sm text-gray-500 col-span-2 text-center mt-4">Belum ada bingkai studio.</p>';
                }
            } catch (err) {
                console.error(err);
            }
        ...
        function updateCaptureUI() {
            const totalSlots = (currentFrame && currentFrame.slots && Array.isArray(currentFrame.slots) && currentFrame
                .slots.length > 0) ? currentFrame.slots.length : 1;

            // Tampilkan tombol back jika di pose kedua atau lebih
            if (multiTakeIndex > 0) {
                prevSlotBtn.classList.remove('hidden');
            } else {
        ...
        function selectFrame(index, element) {
            document.querySelectorAll('.frame-item').forEach(el => el.classList.remove('active', 'ring-4',
                'ring-blue-300'));
            ...
            overlayFrame.onload = () => {
                if (overlayFrame.naturalWidth > 0 && overlayFrame.naturalHeight > 0) {
        ...
        function renderLiveStudio() {
            ...
            if (liveStudioCanvas.width !== SW || liveStudioCanvas.height !== SH) {
                liveStudioCanvas.width = SW;
                liveStudioCanvas.height = SH;
            }
        ...
        captureBtn.addEventListener('click', () => {
            ...
            prevSlotBtn.classList.add('hidden'); // Sembunyikan navigasi saat review
        ...
        function acceptCurrentSlot() {
            multiTakeIndex++;
            let totalSlots = (currentFrame.slots && Array.isArray(currentFrame.slots) && currentFrame.slots.length > 0) ?
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = (e) => {
                    ...
                    fabric.Image.fromURL(e.target.result, function(img) {
                        ...
                        const actualH = img.height * scale;
        ...
        function addSlot() {
            ...
            const ch = editorCanvas.actualH || 400;
        ...
        document.getElementById('upload-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (res.ok) {
                    await res.json();
                    alert('Sukses! Bingkai Polaroid baru telah tersedia.');
                    closeUploadModal();
                    ...
                    if (res.status === 401) {
                        alert('Oops! Sesi Login Anda telah terputus.');
